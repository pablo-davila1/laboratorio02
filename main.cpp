...
int exp(int base, int exponente);//prototipo de la fncion

void to_char( int num);//prototipo de la fncion
...
while (problema != 0){
  ...
  switch (problema) {
      ...
      */
      ...
      case 5:{
      /*
      5. Haga una función que reciba un numero entero (int) y lo convierta a cadena de caracteres. Use
...
int comversoraint(char number[15]){
    ...
    for (int i = 0; size != 0 ; i++){
        if(*(numbers+i) == '1'){
            entero += 1*exp(10,size-1);
        }
        else if(*(numbers+i) == '2'){
            entero += 2*exp(10,size-1);
        }
        else if(*(numbers+i) == '3'){
            entero += 3*exp(10,size-1);
        }
        else if(*(numbers+i) == '4'){
            entero += 4*exp(10,size-1);
        }
        else if(*(numbers+i) == '5'){
            entero += 5*exp(10,size-1);
        }
        else if(*(numbers+i) == '6'){
            entero += 6*exp(10,size-1);
        }
        else if(*(numbers+i) == '7'){
            entero += 7*exp(10,size-1);
        }
        else if(*(numbers+i) == '8'){
            entero += 8*exp(10,size-1);
        }
        else if (*(numbers+i) == '9')
            entero += 9*exp(10,size-1);
        else if (*(numbers+i) == '0')
            entero += 0;
...
// esta funcion espera un entero, lo comvierte a char y lo muestra en pantalla
void to_char( int num){

    int cant = 0;
    int numsize= num;
    //int numcop = 0;
    for (int i=10 ; numsize > 0 ; i+=0){
        cant++;
        numsize = numsize/i;
    }

    //cadnum = new char[cant];
    char cadnum[cant];
